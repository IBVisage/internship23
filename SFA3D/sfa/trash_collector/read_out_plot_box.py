...
from utils.visualization_utils import merge_rgb_to_bev, show_rgb_image_with_boxes, corners_of_boxes
...
def process_single_image(img_id):
    ...
    filtered_data = kalman(outputs)

    if len(filtered_data) > 0:
        filtered_data[:, 1:] = lidar_to_camera_box(outputs[:, 1:], calib.V2C, calib.R0, calib.P2)
        corners_out = corners_of_boxes(filtered_data, calib)

    print(corners_out)

    image_path = "../dataset/kitti/testing/image_2/video_" + video_num + "/" + img_id + ".png"
